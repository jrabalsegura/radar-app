...
La Fase 7 ejecuta dos pipelines independientes: la composición nacional
`national-v1` de Península y Baleares y el pipeline configurable de los 15
radares regionales. Cada producto tiene manifiesto, estado y cobertura propios.
Una fuente temporalmente sin imagen conserva un manifiesto válido y sigue
consultándose.
...
.venv/bin/aemet-radar validate-national ruta/al/original.png
...
`run` consulta una vez por ciclo las cronologías nacional `compo/PB` y regional
PPI, archiva hasta 24 observaciones por producto y después procesa solo
novedades. Si una cronología o la observación más reciente no son utilizables,
consulta OpenData. Una lámina de indisponibilidad no es una observación.
...
genera también cualquier derivado nacional o regional que falte en la ventana
pública. El servidor escucha solo en `127.0.0.1` por defecto y no permite
listar directorios.
...
La composición nacional valida PNG indexado `962×1079`, 4 bits y límites
EPSG:3857. `validate-national` genera `mask.png`, `overlay.png` y
`national-processing.json`; la máscara conserva únicamente RGB de
reflectividad y no usa una silueta de cobertura fija:
...
`docs/PHASE_6.md`; el contrato nacional está en `docs/PHASE_7.md`.
